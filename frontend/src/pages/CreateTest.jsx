...
function CreateTest(){
    ...
    <>
          ...
          <div className="px-5 h-[90vh] overflow-y-auto">
               ...
               <form className="border shadow-lg rounded p-5 px-20 flex flex-col gap-4 cursor-pointer">
                    ...
                    <textarea onChange={(e)=>{setData((pre)=>{setError({test:'',que:''}); return{...pre,dis:e.target.value}})}} value={data.dis} className="p-2 border rounded shadow" rows="6" name="" id="dis"></textarea>
                             ...
                             <div className="border shadow-lg rounded p-5 cursor-pointer">
                                <p className="text-center font-semibold mb-6 text-lg">Questions</p>
                                {data.questions.map((ques,idx)=>{
                                    return <div className="flex justify-between gap-5 my-1 w-[60%]">
                                        <p className="py-1 px-2 font-semibold" key={idx}>Q.{idx+1} {ques.que}</p>
                                        <button onClick={(e)=>{e.preventDefault(); return handelDelete(idx); }} className="p-1 px-4 bg-[#01b4dc] font-semibold text-white rounded w-[5rem]">delete</button>
                                    </div>
                                })}
                             </div>
